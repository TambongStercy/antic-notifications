...
export class NotificationService {
    ...
    public getProviders() {
        return { whatsapp: this.whatsapp, telegram: this.telegram };
    }
    ...
    public async sendTelegram(request: NotificationRequest) {
        if (!this.telegram.isConnected()) {
            return { success: false, error: 'Telegram bot not configured or connected' };
        }
    ...
    public async health(version: string): Promise<HealthCheckResponse> {
        ...
        const overallHealthy = dbHealth.status === 'connected' &&
            ['connected', 'authenticating', 'not_configured'].includes(services.whatsapp) &&
        ...
        return {
            ...
            services: {
                ...
                whatsapp: services.whatsapp === 'not_configured' ? 'not_configured' :
                    services.whatsapp === 'authenticating' ? 'authenticating' :
